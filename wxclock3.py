...
"""
import os,sys,os.path,math,random
import wx
...
class kclock(wx.Frame,FPSlogic):
    ...
    def resizefixaspect( self, image, wx,wy  ):
        ...
        rtnimg = None
        try :
            rtnimg = image.Scale( oriw * scalerate , orih*scalerate  ).Size( (wx,wy), ( (wx-oriw * scalerate )/2  , (wy-orih * scalerate )/2) )
        except :
            pass
        return rtnimg
    ...
    def _recalcCoords(self, size):
        if self.rawbgimage :
            self.bgbitmap = self.resizefixaspect( self.rawbgimage,size.width, size.height ).ConvertToBitmap()
        else :
    ...
    def _printText( self,dc, pstr, x, y ,r=True,g=True,b=True, depth = 2):
        ...
        for i in range(0,depth) :
            ...
            dc.SetTextForeground(
                    wx.Colour( cr if r else cr*.9 ,cr if g else cr*.9  , cr if b else cr*.9, 0x80 )
                    )
    ...
    def getCenterPos(self, even = True ):
        ...
        centerlen = self.maxlen + (self.clientsize[0] - self.maxlen *2)* ( lt.tm_sec *100 + ms/10.0 ) / 6000.0
        if lt.tm_min % 2 :
            rtnx = self.clientsize[0] - centerlen if even else centerlen
        else :
            rtnx = centerlen if even else self.clientsize[0] - centerlen

        centerlen = self.maxlen + (self.clientsize[1] - self.maxlen *2)* ( lt.tm_sec *100 + ms/10.0 ) / 6000.0
        if lt.tm_min % 2 :
    ...
    def _drawCalendar(self,dc):
        ...
        calday = calendar.Calendar(6).monthdays2calendar(localtime()[0], localtime()[1])
    ...
    def _drawclockhands( self,dc,mst , wday):
        ...
        if True :
            ...
            mincx , mincy  = self.getpoint2( seccx, seccy , self.maxlen, secangle, 0.2 )
            hourcx , hourcy = self.getpoint2( mincx , mincy , self.maxlen, minangle, 0.2 )
        else :
            hourlen , minlen, seclen = 1.0 , 0.8 , 0.6

            hourcx , hourcy = wdposx, wdposy #self.mcentery
            mincx , mincy  = self.getpoint2( hourcx , hourcy , self.maxlen, hourangle, 0.2 )
            seccx, seccy = self.getpoint2( mincx , mincy , self.maxlen, minangle, 0.2 )


        hourpos = self.getpoint2( hourcx , hourcy , self.maxlen, hourangle, hourlen )
        ...
        self._drawClockTicks2(dc, hourcx , hourcy,self.maxlen*hourlen, hourco )#, "black" )
        ...
        self._drawClockTicks2(dc, mincx , mincy,self.maxlen*minlen, minco)#, "black" )
        ...
        self._drawClockTicks2(dc, seccx, seccy,self.maxlen*seclen, secco)#, "black" )
