...
import os,sys,os.path,time,math,random
import wx
...
def makeSizeList2( slist ):
    ...
    for i,carg in zip( range(len(slist) ), slist) :
        if type(carg) is int :
            ...
            raise exceptions.TypeError('invalid type %s type must list or tuple' % type(carg) )
    return rtn
...
class kclock(wx.Frame,FPSlogic):
    def __init__(self, *args, **kwds):
        self.fold = True
        self.foldstep = 0
        self.winSizeList = makeSizeList2( ( (512, 32) ,  10,  (512, 16*(20+worldTime.getCount()) ) )  )
    ...
    def _OnSize(self, evt):
        ...
        if self.clientsize[0] < 1 or self.clientsize[1] < 1:
            return
        self.adj = self.clientsize[0]/ 32 # min(self.clientsize[0]/6,self.clientsize[1]/3)
    ...
    def _OnPaint(self, evt):
        ...
        dc.SetPen( wx.Pen(wx.Colour( 0x7f , 0x7f , 0x7f ), 0) )
        dc.SetBrush( wx.Brush( wx.Colour( 0xff  , 0xff * (1-cper), 0xff * (1-cper) ) , wx.SOLID) )
        dc.DrawRectangle( 0,0, self.clientsize[0] * cper , 1.1 * self.adj )

        minfo = meminfo()
        mtot , mfree, mbuf, mcach = minfo['MemTotal'] , minfo['MemFree'] , minfo['Buffers'] , minfo['Cached']
        mper = 1-(mfree + mbuf + mcach)*1.0/mtot
        dc.SetPen( wx.Pen(wx.Colour( 0x7f , 0x7f , 0x7f ), 0) )
        dc.SetBrush( wx.Brush( wx.Colour( 0xff * (1-mper) , 0xff , 0xaf* (1-mper) ) , wx.SOLID) )
        dc.DrawRectangle( 0,1.1 * self.adj, self.clientsize[0] * mper , 1.1 * self.adj )
        ...
        self._printText( dc,today.strftime("%H:%M:%S"),self.adj*6 , 1.1 * self.adj)

        dc.SetFont(self.calfont)
        #datetext = strftime("%a %Y-%m-%d", today)
        self._printText( dc,today.strftime("%a %Y-%m-%d"),self.adj*22 , 0.55 * self.adj)
        ...
        if self.foldstep > 0 :
            ...
            #datetext = clocks[i][0] + clocks[i][1]( clocks[i][2]).strftime(datetimeformatstring )
                ...
                self._printText( dc,datetext,self.adj*16 , self.adj * ( 18 + i*2 ) , 0x6f, 0x6f, 0x6f )
